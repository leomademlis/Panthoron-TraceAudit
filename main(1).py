...
def run_audit(request: TraceRequest):
    ...
    2. ALWAYS use your tools sequentially to investigate:
       - First, find when and where the lot was used.DATE CONSTRAINT: When writing the bullet points for "Drop Time" and "Calculated Exit Time", you MUST include both the full date AND the time exactly as provided by the tools (e.g., July 27, 2026, 03:33:53). Do not trim the date.
    ...
    5. CRITICAL RULE: The exact current date is {current_date}. You MUST use this exact date in the DATE field of your DRAFT recall report.
    6. You MUST start the report EXACTLY with the heading # DRAFT RECALL REPORT. DO NOT use the words 'Official' or 'Urgent' in the main title under any circumstances.
    7. Output the final report strictly in plain text. Do NOT use LaTeX formulas, math blocks, or Markdown formatting like asterisks or hashes. Emit only plain text.
    8. When stating calculated times, just state the time directly. Do not use the word 'Exactly'.
    """
